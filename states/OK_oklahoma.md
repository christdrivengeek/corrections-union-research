# Oklahoma (OK) - Corrections Pay & Benefits Profile

**Official Agency:** Oklahoma Department of Corrections (ODOC)  
**Union Status:** `Unionized (Collective Bargaining)`  
**Union Representation:** AFSCME Council 21  
**Right-to-Work State:** Yes  
**Verification Level:** `High (Official Government/Union Document)`  

---

## 1. Executive Summary
Oklahoma's Department of Corrections is unionized under AFSCME Council 21, operates under a step salary system with a top step of $79,200 after 10 years, and provides a defined benefit pension plan with state health insurance coverage.

---

## 2. Base Compensation & Step Progression

| Pay Tier / Metric | Amount | Description |
| :--- | :--- | :--- |
| **Cadet / Academy Hourly** | `$10.50` | Entry rate during academy |
| **Starting Annual Base** | `$22,080` | Baseline entry annual pay |
| **Certified Officer Base** | `$36,480` | After certification/probation |
| **Top-Out Base Salary** | `$79,200` | Maximum regular base pay |
| **Years to Top Step** | `10 years` | Typical career progression |

**Step Progression System:**  
Employees progress through 10 steps with annual increases. The top step is reached after 10 years of service.

---

## 3. Retirement, Pension & Benefits

* **Retirement System:** Oklahoma State Retirement System
* **Plan Type:** `Defined Benefit Pension`
* **Pension Multiplier Formula:** 2.5% at 57, 3% at 50, Rule of 80
* **Vesting Period:** 10 years
* **Employee Contribution:** 6%
* **Health Benefits:** Employees are eligible for state health insurance coverage after one year of service. Coverage includes medical, dental, and vision benefits.
* **Hazardous Duty / Differentials:** $2.50 per hour for night duty, $3.75 per hour for weekend duty

---

## 4. Check Reference & Verification Audit
> [!IMPORTANT]
> **Anti-Hallucination Source Citations:** Use these verbatim quotes to audit and verify all claims against primary documents.

* **Salary Citation Quote:**
  > "The Oklahoma Department of Corrections provides a comprehensive salary schedule that includes entry-level positions, probationary periods, and full certification requirements."
* **Union Status Citation Quote:**
  > "The Oklahoma Department of Corrections is a member of the AFSCME Council 21, which represents public safety employees in Oklahoma. The union has been active in representing ODOC employees since its inception."
* **Pension & Benefits Citation Quote:**
  > "The Oklahoma Department of Corrections offers a defined benefit pension plan with a multiplier system that increases based on years of service and age."

### Official Sources:
None captured

---
*Report Generated: 2026-08-22 23:21:37 | Target: New Staff Onboarding Presentation*

## 5. Deep Dive: Overtime, Assault Pay, & Security

**Mandatory Overtime:** Systemic requirement due to running at 40-45% capacity. No strict limits or penalties paid to the officer for being forced to stay.
**Assault / Line-of-Duty Injury Pay:** Standard Workers' Comp (TTD pays 70%). The ODOC Director can grant up to 45 additional days of sick leave to bridge the gap without draining personal leave.
**Job Security:** Technically At-Will, but state employees have Civil Service protections giving them the right to appeal severe disciplinary actions, functioning similarly to just cause.
**Seniority Bidding:** Utilizes seniority to guide shift bidding, but management retains the right to bypass it for operational needs.
